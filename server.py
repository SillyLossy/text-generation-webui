...
parser.add_argument('--memory', action='store_true', help='Simulates a long-term memory in chat UI modes using BART summarization model.')
parser.add_argument('--summarization-model', type=str, help='Huggingface model name of a BART summarization model a memory system will use.')
...
if args.memory and (args.cai_chat or args.chat):
    if args.summarization_model is not None:
        summarization_model = args.summarization_model
    else:
        summarization_model = "sshleifer/distilbart-cnn-12-6"
    import modules.bot_memory as bot_memory
    bot_memory.init(summarization_model)
...
def chatbot_wrapper(text, tokens, do_sample, max_new_tokens, temperature, top_p, typical_p, repetition_penalty, top_k, min_length, no_repeat_ngram_size, num_beams, penalty_alpha, length_penalty, early_stopping, name1, name2, context, check, chat_prompt_size, picture=None):
    ...
    if args.picture and picture is not None:
        ...
        visible_text = text
    if args.memory:
        bot_memory.state['short_buffer'].append(['', ''])
...
def save_history(timestamp=True):
    ...
    with open(Path(f'logs/{fname}'), 'w') as f:
        f.write(json.dumps({'data': history['internal'], 'data_visible': history['visible'], 'memory': memory}))
    ...
    try:
        ...
        # Compatibility with Pygmalion AI's official web UI
            ...
            if len(j['chat']) > 0 and j['chat'][0].startswith(f'{name2}:'):
                ...
                history['visible'] = copy.deepcopy(history['internal'])
        if args.memory and 'memory' in j:
            bot_memory.state = j['memory']
    except:
